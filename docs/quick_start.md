...
## 12. Assess publication bias (Cells 16 – 19)

Publication bias is the systematic under-reporting of studies with non-significant or unexpected results. Its effect on a meta-analysis is real and one-directional: the studies that *did* get published lean toward stronger effects than the population of all studies that were actually conducted, so the pooled estimate is inflated by a bias the data cannot show you directly. Detecting and — where possible — adjusting for publication bias is a standard component of any rigorous meta-analytical manuscript, and reviewers will expect to see it.

CoMeta implements four complementary diagnostics distributed across four cells. They are designed to be reported together; no single one is decisive, and the responsible practice is to run all four.

### Why this matters

The intuition behind every publication-bias diagnostic is the **funnel plot**: a scatter of effect size against precision (or its inverse, the standard error). In an unbiased literature, large studies sit at the narrow top of the funnel close to the true effect, while smaller studies spread out symmetrically below. If small studies preferentially exist *only* when they produce large effects — because small studies with null results were left in the file drawer — the bottom of the funnel becomes asymmetric. That asymmetry is the empirical signature of publication bias (though several other mechanisms can produce it; see the caveats below).

The four diagnostics ask the same question — *is the funnel asymmetric?* — in complementary ways:

- **Egger's regression test** (Cell 16) — the formal statistical test of asymmetry. A non-zero intercept means the funnel is leaning.
- **Trim-and-fill** (Cells 16 & 17) — non-parametric and "what-if": imputes the studies that *would* be there if the funnel were symmetric and reports what the pooled estimate *would* be with them included. The gap between the original and adjusted estimate is your sensitivity bound.
- **PET-PEESE** (Cell 18) — model-based: fits a regression that explicitly models the small-study effect and reports a bias-corrected pooled estimate.
- **Funnel plot** (Cell 19) — the figure that lets a reader judge the diagnostics visually. Always include it.

### Run the four cells

**Cell 16 — Egger's test and trim-and-fill.** Click **▶ Run Publication Bias Analysis**. Five tabs are populated:

- **📊 Egger's Test** — the intercept, *t*-statistic, and *p*-value. A *p* < 0.05 indicates funnel asymmetry; a *p* above this is *not* evidence of the absence of bias, only of its detectability.
- **📉 Trim-and-Fill** — the number of studies imputed, the original pooled estimate (from Cell 7), and the adjusted estimate side-by-side. A non-trivial gap suggests bias.
- **🔍 Combined Assessment** — a qualitative synthesis of both tests, intended as the starting point for your *Results* paragraph.
- **📝 Publication Text** and **💾 Export** — manuscript text and an Excel audit workbook.

**Cell 17 — Trim-and-fill plot.** Generates the funnel-style figure that overlays the observed studies and the trim-and-fill-imputed studies, with vertical reference lines at the original and adjusted pooled estimates. The visible gap between the two reference lines communicates the magnitude of the adjustment at a glance. Customise across seven tabs and click **Generate Plot**.

**Cell 18 — PET-PEESE.** Adjust the **P-threshold:** slider (default 0.10) if your conventions differ — this is the *α* level at which the PET intercept is judged significant, used to decide whether PET or PEESE provides the bias-corrected estimate — then click **▶ Run PET-PEESE**. Four tabs report the PET intercept, the PEESE intercept, the selected bias-corrected estimate, and an interpretation paragraph.

**Cell 19 — Funnel plot.** The canonical funnel plot, ready for the manuscript. The **▼ Funnel** tab offers three useful overlays:

- **Show Significance Contours (1 %, 5 %, 10 %)** — distinguishes genuine asymmetry from sampling noise.
- **Fill Confidence Region** — shades the 95 % funnel envelope.
- **Show Egger's Test Stats** — annotates the figure with the intercept and *p*-value from Cell 16.

You can also use **Color By:** to colour points by a categorical moderator. Click **📊 Generate Funnel Plot**.

### How to report it

The convention is to report Egger's test as the primary inferential test, the trim-and-fill adjusted estimate as a sensitivity check, and the PET-PEESE estimate as an additional bias-corrected estimate — all accompanied by the funnel plot of Cell 19. Each of the four cells produces its own autogenerated text or caption, which together cover the publication-bias paragraph in your *Results* section.

### Important caveats

- **Funnel asymmetry has many causes besides publication bias.** Small studies often differ from large ones in genuine ways (different methods, different study populations, different quality), and any of these can produce asymmetry even in the absence of selective reporting. Do not declare publication bias from a single significant Egger's test alone — discuss the alternative explanations.
- **Trim-and-fill rests on a strong symmetry assumption.** The adjusted estimate is what would obtain *if* the missingness mechanism produces exact left–right symmetry, which is rarely met under heterogeneity. Report it as a sensitivity bound, not a corrected estimate.
- **Report all four, or justify the omission.** Reporting only the test that comes back favourable is the most common form of selective reporting in published meta-analyses. The defensible practice is to report all four.
